...
abstract class PlaylistSupabaseService {
  ...
  Future<Either> updatePlaylistInfo(
      String playlistId, String title, String desc);
  ...
  Future<Either> batchAddToPlaylist(
      String playlistId, List<SongWithFavorite> songList);
}
...
class PlaylistSupabaseServiceImpl extends PlaylistSupabaseService {
  ...
  Future<Either> getCurrentUserPlaylists(String userId) async {
    try {
      ...
      var data = await supabase.from('playlists').select('*').match(
          {'user_id': userId.isEmpty ? supabase.auth.currentUser!.id : userId});
      ...
      for (final playlistModel in list) {
        if (list.isNotEmpty) {
          var songCount = await supabase
              .from('playlist_songs')
              .select('*')
              .match({'playlist_id': playlistModel.id!}).count();
  ...
  @override
  Future<Either<String, PlaylistEntity>> addNewPlaylist(String title,
      String description, bool isPublic, List selectedSongId) async {
    try {
  ...
  @override
  Future<Either> updatePlaylistInfo(
      String playlistId, String title, String desc) async {
    ...
    try {
      await supabase
          .from('playlists')
          .update({'name': title, 'description': desc}).eq('id', playlistId);
  ...
  @override
  Future<Either> addSongsToPlaylist(
      String playlistId, List selectedSongId) async {
    String message = '';
  ...
  Future<Either> addSongByKeyword(String playlistId, String title) async {
    try {
      ...
      if (result.isNotEmpty) {
        songEntity =
            SongWithFavorite(SongModel.fromJson(result).toEntity(), true);

        await supabase
            .from('playlist_songs')
            .insert({'playlist_id': playlistId, 'song_id': songEntity.song.id});
  ...
  @override
  Future<Either> batchAddToPlaylist(
      String playlistId, List<SongWithFavorite> songList) async {
    try {
      if (songList.isEmpty) {
